...
indic <- eurostat::get_eurostat("demo_r_pjanind3",
    time_format = "num"
) %>%
    dplyr::filter(
        indic_de == "PC_Y0_4" &
...
d <- nuts3 %>% left_join(df, by = "NUTS_ID")
head(d)
...
p <- ggplot() +
    ...
    scale_fill_manual(
        name = "",
        values = c(
            "#f0e771", "#77c8a9", "#289f9e",
            "#406ba0", "#4a26ad", "grey80"
        ),
        drop = F
    ) +
    ...
    theme(
        ...
        plot.title = element_text(
            face = "bold", size = 100, color = "#1fada0", hjust = .5
        ),
    ...
    labs(
        ...
        y = NULL,
        title = "% of population aged 0-4 in 2021*",
        subtitle = "",
        caption = "* The data for Croatia and Serbia is from 2020"
    )

ggsave(
    filename = "eur_kids_2021.png",
