...
indic <- eurostat::get_eurostat("isoc_r_iuse_i",
    time_format = "num"
) %>%
    dplyr::filter(
        indic_is == "I_IUBK" &
...
p <- ggplot() +
    ...
    scale_fill_manual(
        name = "",
        values = c(
            "#ffe1a8", "#eeb188", "#d0866e",
            "#a96258", "#7a4445", "#472d30", "grey80"
        ),
        drop = F
    ) +
    ...
    theme(
        ...
        plot.title = element_text(
            face = "bold", size = 100, color = "#723d46", hjust = .5,
            lineheight = .2
        ),
        plot.caption = element_text(
            size = 50, color = "grey20", hjust = .5, vjust = 12
        ),
    ...
    labs(
        x = "©2022 Milos Popovic (https://milospopovic.net)\nData: https://ec.europa.eu/eurostat/databrowser/view/ISOC_R_IUSE_I__custom_4538780/default/table?lang=en",
        y = NULL,
        title = "% of people who used the Internet\nfor online banking in 2022*",
        subtitle = "",
        caption = "* The data for Iceland, Ireland, N. Macedonia, Bosnia-Herzegovina and Switzerland is from 2021"
    )

ggsave(
    filename = "eur_online_banking_2022.png",
